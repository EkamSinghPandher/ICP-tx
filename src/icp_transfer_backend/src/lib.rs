...
use ic_cdk_macros::*;
use ic_ledger_types::{
    AccountIdentifier, BlockIndex, Memo, Subaccount, Timestamp, Tokens, DEFAULT_SUBACCOUNT, MAINNET_LEDGER_CANISTER_ID
};
...
async fn transfer(args: TransferArgs) -> Result<BlockIndex, String> {
    ...
    let timestamp:Option<Timestamp> = Some(Timestamp{timestamp_nanos: args.time});
